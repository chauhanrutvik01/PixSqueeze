...
import { compressionLinks, sizeSlugs, toolLinks } from "@/lib/site";

export function Footer() {
  return (
    <footer className="mt-24 border-t border-line bg-white/70">
      <div className="container-shell grid gap-12 py-14 sm:grid-cols-2 lg:grid-cols-[1.2fr_1fr_1fr_1fr]">
        <div>
          <Logo />
        ...
        <div>
          <h2 className="text-sm font-extrabold uppercase tracking-widest text-ink">Compress formats</h2>
          <div className="mt-4 grid gap-2 text-sm text-slate-600">
            {compressionLinks.map((tool) => <Link className="hover:text-moss" key={tool.href} href={tool.href}>{tool.label}</Link>)}
            <Link className="hover:text-moss" href="/check">Check image size</Link>
          </div>
        </div>
        <div>
          ...
          <div className="mt-4 grid gap-2 text-sm text-slate-600">
            ...
            <Link className="hover:text-moss" href="/compress-images-for-website-speed">Images for website speed</Link>
